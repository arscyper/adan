...
public class PrayTime {
    ...
    public ArrayList<String> timeNamesAr;
    ...
    public PrayTime() {
        ...
        timeNamesAr = new ArrayList<String>();
        timeNamesAr.add("\u0627\u0644\u0641\u062C\u0631");
        timeNamesAr.add("\u0634\u0631\u0648\u0642 \u0627\u0644\u0634\u0645\u0633");
        timeNamesAr.add("\u0627\u0644\u0638\u0647\u0631");
        timeNamesAr.add("\u0627\u0644\u0639\u0635\u0631");
        timeNamesAr.add("\u0627\u0644\u063A\u0631\u0648\u0628");
        timeNamesAr.add("\u0627\u0644\u0645\u063A\u0631\u0628");
        timeNamesAr.add("\u0627\u0644\u0639\u0634\u0627\u0621");
...
/*        int year = date.get(Calendar.YEAR);
	...
	public String print(int day){
	    ...
	    //isoFormat.setTimeZone(TimeZone.getTimeZone(dto.getZoneName()));	
	    SimpleDateFormat isoFormatPartial = new SimpleDateFormat("yyyy-MM-dd'T'");
	    Calendar cal = Calendar.getInstance();
	    
	    if(!"unknown".equals(dto.getZoneName())){
		    isoFormatPartial.setTimeZone(TimeZone.getTimeZone(dto.getZoneName()));
		    cal.setTimeZone(TimeZone.getTimeZone(dto.getZoneName()));
	    }
	    
	    //cal.setTime(date);
	    cal.add(Calendar.DAY_OF_MONTH, day);
	    
        int zone_id = (cal.get(Calendar.ZONE_OFFSET) + cal.get(Calendar.DST_OFFSET)) / (60 * 60 * 1000);//cal.get(Calendar.ZONE_OFFSET)/(1000*60*60);
							...
							"<td>" +
						 	...
						 	"<div itemprop=\"location\" itemscope itemtype=\"http://schema.org/Place\">" +
							    "<div itemprop=\"address\" itemscope itemtype=\"http://schema.org/PostalAddress\">"+
							      ...
							      "<meta itemprop=\"addressRegion\" content=\""+dto.getState()+"\">"+
							    "</div>"+		
							"</div>"+
						 	"</td>" +
	...
	public String[] getMonthInfo(int i){
		...
		try{
		    ...
		    fmt.setTimeZone(TimeZone.getTimeZone(dto.getZoneName()));	
			...
			String s1[] = fmt.format(cal.getTime()).split("/");
			return s1;
	...
	public static int getZone(String s){
        ...
        if(s.startsWith("-")){
        	if(s.charAt(1) == '0'){
        		timezone = Integer.valueOf(Character.toString(s.charAt(2))) * -1;
        	}else{
        		timezone = Integer.valueOf(s.substring(0, 3));
        	}
        }else if(s.startsWith("+")){
        	if(s.substring(1).startsWith("0")){
        		timezone = Integer.valueOf(Character.toString(s.charAt(2)));
        	}else{
        		timezone = Integer.valueOf(s.substring(1, 3));
        	}
        }else{
        	if(s.startsWith("0")){
        		timezone = Integer.valueOf(Character.toString(s.charAt(1)));
        	}else{
        		timezone = Integer.valueOf(s.substring(0, 2));
        	}
        }
        return timezone;
    }
	...
	public void setLocationDto(LocationDto dto){
		...
		if(dto.getZoneName() == null){
			dto.setZoneName(com.app.timezone.TimeZoneMapper.getZoneName(dto.getLatitude(), dto.getLongitude()));
		}else if(dto.getZoneName().length() < 3){
			dto.setZoneName(com.app.timezone.TimeZoneMapper.getZoneName(dto.getLatitude(), dto.getLongitude()));
		}
		//System.out.println(dto.getZoneName());
	}
